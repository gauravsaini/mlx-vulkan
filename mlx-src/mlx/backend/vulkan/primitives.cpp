...
void dispatch_affine_qmv_transpose_medium(
    const array& x,
    const array& w,
    const array& scales,
    const array& biases,
  ...
  if (pipeline == VK_NULL_HANDLE) {
    throw std::runtime_error(
        "[QuantizedMatmul::eval_gpu] Missing Vulkan quantized_qmv_medium pipeline.");
  }
  ...
  if (debug_vulkan_qmm()) {
    std::fprintf(
        stderr,
        "[qmm] affine gpu medium_qmv=1 transpose=1 bits=%d group_size=%d x_ndim=%zu w_ndim=%zu out_ndim=%zu M=%u K=%u N=%u\n",
        bits,
        group_size,
        x.ndim(),
        w.ndim(),
  ...
  vkCmdBindDescriptorSets(
      cmd, VK_PIPELINE_BIND_POINT_COMPUTE, layout, 0, 1, &ds, 0, nullptr);
  vkCmdDispatch(cmd, vulkan::div_ceil(N, 4u), M, 1);
...
void QuantizedMatmul::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  bool use_medium_qmv = transpose_ && w.ndim() == 2 && M == 1 &&
      group_size_ == 64 && (bits_ == 4 || bits_ == 5) &&
      (K == 2048u || K == 6144u) && (N >= 512u && N <= 6144u) &&
      (N % 8u == 0u);
  if (use_direct_qmv) {
    std::optional<array> temp_out;
  ...
  if (use_medium_qmv) {
    ...
    dispatch_affine_qmv_transpose_medium(
        x, w, scales, biases, *kernel_out, group_size_, bits_, s);
    if (temp_out) {
      out.set_data(allocator::malloc(out.nbytes()));
      copy_gpu(*temp_out, out, CopyType::General, s);
