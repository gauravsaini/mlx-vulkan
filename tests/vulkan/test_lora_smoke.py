#!/usr/bin/env python3
"""
LoRA / small-LLM finetuning smoke for the Vulkan backend.

This extends the training ladder by verifying that LoRA (Low-Rank Adaptation)
parameter updates and gradients work correctly on the Vulkan backend without
falling back to the CPU (for the training loop).
"""
...
def main():
    ...
    print("========================================")
    print("  LoRA Finetuning Vulkan Smoke")
    print("========================================")
    ...
    class LoRALinear(nn.Module):
        def __init__(self, linear_layer: nn.Linear, r: int = 4, alpha: float = 8.0):
            super().__init__()
            ...
            # Initialize B to zero so initially LoRA acts as identity
    ...
    def check_training():
        ...
        # 2. Inject LoRA adapters into q_proj and v_proj
        for block in model.blocks:
            block.q_proj = LoRALinear(block.q_proj)
            block.v_proj = LoRALinear(block.v_proj)
            
        # Verify trainable parameters are only LoRA weights
        trainable = model.trainable_parameters()
        for k in utils.tree_flatten(trainable):
            if not ("lora_a.weight" in k[0] or "lora_b.weight" in k[0]):
                raise RuntimeError(f"Unexpected trainable parameter: {k[0]}")

        # Use larger LR for LoRA finetuning
    ...
    try:
        ...
        check("lora finetuning smoke", check_training, errors)
    finally:
    ...
    print("PASS: LoRA finetuning Vulkan smoke succeeded")
    return 0
