...
- [~] **A native Vulkan medium-decoder QMV specialization now improves the repeated `2048/6144` transpose QMM set on RX 580**:
      ...
      - `mlx/backend/vulkan/kernels/quantized_qmv_medium.comp` now also has a real-Qwen fast path for `5-bit`, `group_size=64`, `values_per_thread == 16`, so the hottest decoder path no longer pays the generic per-element unpack loop
      - the representative layer-13 decode block now improves across the whole expensive projection family:
...
- [~] **Post-`4b378b9` RX 580 profiling shows the next steady-state decode wall is the tied logits path plus the attention-layer QMM / reshape front-end, not the medium-kernel reduction tree**:
      ...
      A follow-up retune on the kept medium decoder path did clear the bar. Shrinking `quantized_qmv_medium.comp` from 8 outputs per workgroup to 4 slightly improves the hot `2048/6144` decoder shapes on the RX 580 without changing correctness:
      - strict quantized smoke still passes, including the real `1 x 2048 -> 4096` `q_proj`-class regression
